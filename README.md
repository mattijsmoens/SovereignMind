...
- **Research-Grade:** Fully decoding complex inner monologue (e.g., "Take me to a snowy mountain") currently requires an active, high-resolution fMRI or MEG machine scanning the brain in real-time.
- **Consumer Prototyping:** For immediate prototyping without an fMRI, you can use consumer EEG headsets like the **EMOTIV EPOC X** or **Muse S**. While these cannot decode complex sentences, they *can* detect high-level cognitive states (e.g., spikes in Focus or Relaxation). These spikes can be passed to SovereignMind as simplified vectors to trigger state-changes in the VR environment.
- **The Simulator:** If you have no BCI hardware, you can use the included `mock_data.py` script to simulate both fMRI vectors and biofeedback telemetry for testing purposes.
